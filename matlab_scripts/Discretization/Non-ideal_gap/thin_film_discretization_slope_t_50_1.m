function [ R, delta_V, N_disc ] = thin_film_discretization_slope_1_t_50( L, origin, mesh, cord )
...
for k = 1:N(3)         % Loop through discretized z axis
    for j = 1:N(2)     % Loop through discretized y axis
        for h = 1:N(1) % Loop through discretized x axis    
            x = x1(h) + delta_L/2 - (k-1)*delta_x; % Location of discretized volume centers along x direction
...
R = [R_membrane];
%{
R_back=zeros(4*N(2), 3);
n=1;
for j = 1:N(2)     % Loop through discretized y axis
x = x1(1)- delta_L/2; % Location of discretized volume centers along x direction
y = y1(j)+ delta_L/2; % Location of discretized volume centers along y direction
...
n=n+1;
x = x1(1) - 3*delta_L/2; % Location of discretized volume centers along x direction
R_back(n,1) = x; % Populate location matrix
R_back(n,2) = y; % Populate location matrix
R_back(n,3) = z; % Populate location matrix
n=n+1;
x = x1(1)- delta_L/2 -delta_x; % Location of discretized volume centers along x direction
y = y1(j)+ delta_L/2; % Location of discretized volume centers along y direction
...
R_back(n,3) = z; % Populate location matrix
n=n+1;
x = x1(1)- 3*delta_L/2 -delta_x; % Location of discretized volume centers along x direction
y = y1(j)+ delta_L/2; % Location of discretized volume centers along y direction
z = z1(2)+ delta_L/2; % Location of discretized volume centers along z direction
R_back(n,1) = x; % Populate location matrix
R_back(n,2) = y; % Populate location matrix
