...
back = cd;
saveDir =  'Spectral_discretizations/'; %fullfile(back,'../../library/Non_uniform_spectra');
spectraDir = [saveDir spectra]; %save in DSGF library folder
writematrix(omega, spectraDir);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Split frequencies in multiple files   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Set for 5 files, adjust if necessary
if split == 'Y' 
    % Split spectra into multiple simulations
    spectra_1 = [saveDir material '_' num2str(N_omega/5) '_nonuniform_split_1.csv']; %if local
    writematrix(omega(1:20)', spectra_1);
    spectra_2 = [saveDir material '_' num2str(N_omega/5) '_nonuniform_split_2.csv']; %if local
    writematrix(omega(21:40)', spectra_2);
    spectra_3 = [saveDir material '_' num2str(N_omega/5) '_nonuniform_split_3.csv']; %if local
    writematrix(omega(41:60)', spectra_3);
    spectra_4 = [saveDir material '_' num2str(N_omega/5) '_nonuniform_split_4.csv']; %if local
    writematrix(omega(61:80)', spectra_4);
    spectra_5 = [saveDir material '_' num2str(N_omega/5) '_nonuniform_split_5.csv']; %if local
