...
results_path = 'results/sample_2_subvolumes/SiC_d_1.00e-07/23_11_15_10_12_24';
...
Q_total_subvol = readmatrix([results_path '/Q_t_subvol.csv']); % verify if the name is the same as the file used 
%Q_total_subvol = readmatrix([results_path '/Q_total_subvol.csv']); % verify if the name is the same as the file used 
...
if mesh == 0
[vert, fac] = voxel_power( R, L_sub(1), [], [], [], [], 'heatmap', Q_total_subvol/max_limit, c_limits );
end
if mesh == 1
[vert, fac] = voxel_power( R_1_c, L_sub(1), [], [], [], [], 'heatmap', Q_1_c, c_limits );
[vert, fac] = voxel_power( R_1_r, L_sub(N_subvol_lim(1)+1), [], [], [], [], 'heatmap', Q_1_r, c_limits );
end
if mesh == 2
[vert, fac] = voxel_power( R_1_c, L_sub(1), [], [], [], [], 'heatmap', Q_1_c, c_limits );
[vert, fac] = voxel_power( R_1_r, L_sub(N_subvol_lim(1)+1), [], [], [], [], 'heatmap', Q_1_r, c_limits );
[vert, fac] = voxel_power( R_2_r, L_sub(N_subvol_lim(2)-N_subvol_lim(1)+1), [], [], [], [], 'heatmap',Q_2_r , c_limits );
[vert, fac] = voxel_power( R_2_c, L_sub(N_total), [], [], [], [], 'heatmap', Q_2_c, c_limits );
end
xlabel('x-axis (m)');
ylabel('y-axis (m)');
zlabel('z-axis (m)');
